...
在 Xiaomi Pad 6 Pro 上运行 Kali Linux Rolling 桌面，采用 GNOME 桌面环境与基于上游 Linux 的设备适配内核。[English](README.md)
...
| 磁吸键盘 | Nanosic WN8030 | ✅ 可用 | 普通按键、音量键与重新吸附输入；挂起后恢复未完整覆盖 |
...
| USB 充电器切换后重连 | USB-C / USB gadget | 🟡 部分支持 | 切换充电器后重新连接电脑的可靠性尚未闭合 |
| USB 3.x SuperSpeed | USB 控制器 / PHY | ❌ 不支持 | 当前配置限制为 USB 2.0 High-Speed |
| USB OTG / 主机模式 | USB-C 数据角色切换 | ❌ 不支持 | 当前固定为 peripheral；不能据此承诺 U 盘、USB 键鼠或扩展坞可用 |
